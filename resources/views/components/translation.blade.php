<x-splade-data default="{ {{$name.'_tomato_translations_en'}}: form.{{$name}}?.en, {{$name.'_tomato_translations_ar'}}: form.{{$name}}?.ar, {{$name.'_tomato_translations_fr'}}: form.{{$name}}?.fr }">
    @if($type === 'text')
    <label class="block">
    ...
    "dark:disabled:[-webkit-text-fill-color:theme(colors.gray.400)] dark:disabled:placeholder:[-webkit-text-fill-color:theme(colors.gray.500)] sm:text-sm",
    "sm:leading-6 ps-3 pe-3 focus:ring-2 ring-primary-500 focus:ring-2 focus:ring-primary-500 rounded-lg"
])->implode(' ') }}" v-model="data.{{$name.'_tomato_translations_fr'}}" @input="form.{{$name.'_tomato_translationsfr'}} = data.{{$name.'_tomato_translations_fr'}}" type="text" placeholder="{{$label . ' '. __('FR')}}" />
        </div>
    </label>
